...
base_arg = ' --T 100000 --env ml100k --resultfoldertail _ctr_{} '.format(now)
...
for seed in seed_list:
    ...
    preseed = args.preseed
    print('preprocess')
    os.system("python3 ./preprocess/ml100k_preprocess.py -seed {}".format(preseed))


    condition_arg = '--seed {}'.format(seed)
    os.system('python3 ./real_main.py'+ base_arg + condition_arg)

real_plotting.plotting(resultfoldertail='_ctr_{}'.format(now), env='ml100k', plotting_dict=plotting_dict, fnametail='_{}_all'.format(preseed))