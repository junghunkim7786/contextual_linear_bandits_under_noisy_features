...
def Load_MovieLens(model_tail, data_tail, encoding, num_partial):
    
    X0 = np.load('./datasets/ml-100k/preprocess/X0{}.npy'.format(data_tail))
    X1 = np.load('./datasets/ml-100k/preprocess/X1{}.npy'.format(data_tail))
    
    if not (num_partial is None):
        X0_len = X0.shape[0] 
        X1_len = X1.shape[0]
    ...
    if encoding:
        state_dict = torch.load('./models/ml100k_autoencoder{}.pt'.format(model_tail))
...
class movielens_Env:
    def __init__(self, args):
        ...
        = Load_MovieLens(model_tail=args.model_tail, data_tail=args.data_tail, \
